...
PRODUCT_NAME       := docker-machine-operator
ENFORCE_LICENSE    ?=

GO_PKG   := go.klusters.dev
REPO     := $(notdir $(shell pwd))
BIN      := docker-machine-operator
COMPRESS ?= no
...
clientset:
	@docker run --rm	                    \
		...
		controller-gen                      \
			object:headerFile="hack/license/go.txt" paths="./api/..."
...
uninstall:
	@cd ../installer; \
	helm uninstall docker-machine-operator --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/docker-machine-operator run \
		--v=3 \
		--secure-port=8443 \
